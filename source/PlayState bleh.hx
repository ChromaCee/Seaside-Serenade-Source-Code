...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (sEnding == 'none' && isStoryMode == true)
		{
...
/*				var posX = 400;
		...
		var file:String = Paths.json(songName + '/dialogueEnd'); // Checks for json/Psych Engine dialogue AFTER the final song is complete
		if (OpenFlAssets.exists(file))
		{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				// ^^^^ setting ambient sounds as music makes life a little easier
				...
				case "ocean-blue":
					sEnding = 'ocean blue end';
				//	startDialogue(dialogueJson);
				//	startCountdown();
					

			/*		var whiteScreen:FlxSprite = new FlxSprite(0, 0).makeGraphic(Std.int(FlxG.width * 3), Std.int(FlxG.height * 3), FlxColor.WHITE);
					add(whiteScreen);
					whiteScreen.scrollFactor.set();
					whiteScreen.blend = NORMAL;
					camHUD.visible = false;
					//snapCamFollowToPos(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
					inCutscene = true;

					FlxTween.tween(whiteScreen, {alpha: 0}, 3, {
						startDelay: 0,
						ease: FlxEase.linear,
	...
	public function startEndDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		// TO DO: Make this more flexible, maybe?
		if (dialogueFile.dialogue.length > 0)
		{
			...
			CoolUtil.precacheSound('dialogueClose');
			var doof:DialogueBoxPsych = new DialogueBoxPsych(dialogueFile, song);
			doof.finishThing = forceEndWeek;
			doof.finishThingEnd = true;
			doof.nextDialogueThing = startNextDialogue;
			doof.skipDialogueThing = skipDialogue;
			doof.cameras = [camHUD];
			add(doof);
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (birdsTween != null)
				birdsTween.active = false;
			if (fishiesTween != null)
				fishiesTween.active = false;
			if (jelliesTween != null)
				jelliesTween.active = false;
			if (dolphinsTween != null)
				dolphinsTween.active = false;
			if (sealsTween != null)
				sealsTween.active = false;
		...
		if (isStoryMode)
		{
				if (sEnding == 'none')
				{
					...
					else
					{
						...
						var oceanBlueNext = (SONG.song.toLowerCase() == "white-sands");
							if (oceanBlueNext)
							{
								teleportCutscene();
							}
				...
				else
				{
					switch (sEnding)
					{
						...
						//inCutscene = true;
						startEndDialogue(dialogueJsonEnd);
		...
		new FlxTimer().start(0.002, function(tmr:FlxTimer)
		{
			switch (cs_time)
			{
				...
				case 270:
					...
					FlxTween.tween(whiteScreen, {alpha: 100}, 1, {
						startDelay: 0,
						ease: FlxEase.linear,
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					...
					new FlxTimer().start(1, function(tmr2:FlxTimer)
					{
						...
						FlxTween.tween(whiteScreen, {alpha: 1}, 1, {
							startDelay: 0.1,
							ease: FlxEase.linear,
							onComplete: function(twn:FlxTween)
							{
