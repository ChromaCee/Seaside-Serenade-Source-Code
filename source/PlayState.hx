...
class PlayState extends MusicBeatState
{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				//	teleportCutscene();
					...
					var whiteScreen:FlxSprite = new FlxSprite(-100, -100).makeGraphic(Std.int(FlxG.width * 3), Std.int(FlxG.height * 3), FlxColor.WHITE);
					add(whiteScreen);
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (sealsTween != null)
				sealsTween.active = false;
			if (dolphinsAngle != null)
				dolphinsAngle.active = false;
			if (sealsAngle != null)
				sealsAngle.active = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (sealsTween != null)
				sealsTween.active = true;
			if (dolphinsAngle != null)
				dolphinsAngle.active = true;
			if (sealsAngle != null)
				sealsAngle.active = true;
